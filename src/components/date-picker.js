...
var HAVE_TRIGGER_TYPES = ['date', 'datetime', 'time', 'time-select', 'week', 'month', 'year', 'daterange', 'monthrange', 'yearrange', 'timerange', 'datetimerange', 'dates'];
...
var getYearTimestamp = function getYearTimestamp(time) {
...
/* harmony default export */ var year_tablevue_type_script_lang_js_ = ({
  ...
  computed: {
    ...
    rows: function rows() {
      ...
      var now = getYearTimestamp(new Date());
      ...
      for (var i = 0; i < 3; i++) {
        ...
        for (var j = 0; j < 4; j++) {
          ...
          var index = i * 4 + j;
          var year = (this.startYear + index).toString();
          var time = new Date(year, 0, 1).getTime();
  ...
  methods: {
    ...
    markRange: function markRange(minDate, maxDate) {
      ...
      for (var i = 0, k = rows.length; i < k; i++) {
        ...
        for (var j = 0, l = row.length; j < l; j++) {
          ...
          if (!cell) continue;
          var index = i * 4 + j;
          var time = new Date(this.startYear + index, 0, 1).getTime();
    ...
    handleMouseMove: function handleMouseMove(event) {
      ...
      if (row !== this.lastRow || column !== this.lastColumn) {
        ...
        var endDate = new Date(this.startYear + index, 0, 1);
    ...
    [
      _c(
        "div",
        {
          ...
          staticClass:
            "el-picker-panel el-date-range-picker el-popper is-pasted",
          class: [
            {
              "has-sidebar": _vm.$slots.sidebar || _vm.shortcuts
            },
...
var getStartDate = function getStartDate(date) {
...
var year_rangevue_type_script_lang_js_calcDefaultValue = function calcDefaultValue(defaultValue) {
  if (Array.isArray(defaultValue)) {
    ...
    return [new Date(defaultValue), Object(date_util_["nextYear"])(new Date(defaultValue), 10)];
  } else {
    return [new Date(), Object(date_util_["nextYear"])(new Date(), 10)];
  }
};
/* harmony default export */ var year_rangevue_type_script_lang_js_ = ({
  ...
  data: function data() {
    var leftDate = getStartDate(new Date());
    return {
      popperClass: '',
      value: [],
      defaultValue: null,
      ...
      maxDate: '',
      leftDate: leftDate,
      rightDate: Object(date_util_["nextYear"])(leftDate, 10),
      rangeState: {
        endDate: null,
        selecting: false,
        row: null,
  ...
  watch: {
    value: function value(newVal) {
      if (!newVal) {
        ...
        if (this.minDate) {
          ...
          if (this.unlinkPanels && this.maxDate) {
            var leftDateYear = this.leftDate.getFullYear();
            var maxDateYear = this.maxDate.getFullYear();
            this.rightDate = maxDateYear - 20 < leftDateYear ? Object(date_util_["nextYear"])(this.leftDate, 10) : getStartDate(this.maxDate);
          } else {
            this.rightDate = Object(date_util_["nextYear"])(this.leftDate, 10);
          }
        } else {
          this.leftDate = getStartDate(year_rangevue_type_script_lang_js_calcDefaultValue(this.defaultValue)[0]);
          this.rightDate = Object(date_util_["nextYear"])(this.leftDate, 10);
        }
      }
    },
    defaultValue: function defaultValue(val) {
      if (!Array.isArray(this.value)) {
        ...
        this.rightDate = val && val[1] && left.getFullYear() !== right.getFullYear() && this.unlinkPanels ? getStartDate(right) : Object(date_util_["nextYear"])(this.leftDate, 10);
      }
    }
  },
  ...
  methods: {
    ...
    // leftPrev*, rightNext* need to take care of `unlinkPanels`
    leftPrevYear: function leftPrevYear() {
      this.leftDate = Object(date_util_["prevYear"])(this.leftDate, 10);
      if (!this.unlinkPanels) {
        this.rightDate = Object(date_util_["prevYear"])(this.rightDate, 10);
      }
    },
    rightNextYear: function rightNextYear() {
      if (!this.unlinkPanels) {
        this.leftDate = Object(date_util_["nextYear"])(this.leftDate, 10);
      }
      this.rightDate = Object(date_util_["nextYear"])(this.rightDate, 10);
    },
  ...
  components: { YearTable: year_table, llsButton: button_default.a }
});
